...
const FAQ = [
  ...
  { question: 'Does this include battery replacement?', answer: 'No. Battery replacement cost and timing vary far too much to assume a number, so we leave it out rather than guess. Factor it in separately if you plan to keep the vehicle beyond the warranty.' },
];

export default async function EvVsPetrolPage() {
  const settings = await getSettings();
  const rows = await db.all<any>(
    `SELECT p.id, p.name, p.fuel_type, p.price_min, b.name AS brand_name,
  ...
  const map = (r: any): CalcBike => ({
    id: r.id, label: `${r.brand_name} ${r.name}`, mileage: r.mileage_kmpl,
    range: r.real_world_range_km ?? r.claimed_range_km, battery: r.battery_capacity_kwh, price: r.price_min,
  });
  const petrolBikes = rows.filter((r) => r.fuel_type !== 'electric').map(map);
  const evBikes = rows.filter((r) => r.fuel_type === 'electric').map(map);
  ...
  return (
    <div className="container-xl py-6">
      ...
      <header className="mt-4 max-w-2xl">
        <h1 className="text-2xl font-bold tracking-[-0.03em] sm:text-[32px]">EV vs petrol running cost</h1>
        <p className="mt-1.5 text-sm leading-6 text-ink-mute">
          Pick a petrol bike and an electric model. We use the specs already in our database — no invented figures — and show
          you cost per kilometre, monthly saving and how long the price difference takes to pay back.
        </p>
      </header>

      <div className="mt-6"><EvCalculator petrolBikes={petrolBikes} evBikes={evBikes} defaults={{
