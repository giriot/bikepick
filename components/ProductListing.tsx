...
export async function ProductListing({ category, title, intro, searchParams }: ListingProps) {
  ...
  const [result, brands, bodyTypes, priceRow] = await Promise.all([
    ...
    db.all<any>(
      `SELECT b.slug, b.name, COUNT(p.id) AS count FROM brands b
    ...
    db.all<any>(
      `SELECT DISTINCT p.body_type FROM products p JOIN categories c ON c.id = p.category_id
        WHERE c.slug = ? AND p.body_type IS NOT NULL AND p.status = 'published'`,
      [category],
    ),
        ...
        <div>
          ...
          {result.items.length === 0 ? (
            <Empty
              ...
              body="Try widening your budget or clearing a filter. If a model is missing entirely, an administrator can add it from the admin panel."
              action={<Link href={`/${category}`} className="btn-primary btn-sm mt-2">Clear filters</Link>}
            />
          ) : (
            <>
