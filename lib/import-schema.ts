...
export const IMPORT_TYPES: ImportType[] = [
  {
    ...
    columns: [
      ...
      { name: 'service_interval_km', type: 'number', help: 'Scheduled service distance in km, from the owner manual.' },
      { name: 'est_service_cost', type: 'number', help: 'Rupees per typical service. Estimate only from a verified source — the AI spec queue never writes this column.' },
      { name: 'accessories', type: 'string', help: 'Dealer-fitted or bundled accessories, comma-separated. Never auto-filled; a person decides what is true.' },
      { name: 'range_basis', type: 'string', help: 'Electric only — what the range figure means, e.g. "IDC" / "ARAI" / "real-world".' },
      { name: 'fast_charge_time_min', type: 'number', help: 'Electric only — minutes on a fast charger for the quoted state of charge.' },
      { name: 'running_cost_per_km', type: 'number', help: 'Electric only — rupees per km. Estimate only from a verified source; never auto-filled.' },
      { name: 'est_battery_replacement_cost', type: 'number', help: 'Rupees for a battery replacement. Estimate only from a verified source; never auto-filled.' },
